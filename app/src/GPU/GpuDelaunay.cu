...
void GpuDel::getTriSegments(const Tri &t, Segment *sArr)
{
...
void GpuDel::compute(const GDel2DInput &input, GDel2DOutput *output)
{
    // Set L1 for kernels
...
void GpuDel::constructInitialTriangles()
{
    ...
    _dPredWrapper.init(
        toKernelPtr(_pointVec), _pointNum, _input->noSort ? nullptr : toKernelPtr(_orgPointIdx), _infIdx);
...
void GpuDel::initForFlip()
{
    ...
    _pointNum = static_cast<int>(_input->InputPointVec.size()) + 1; // Plus the infinity point
    _triMax   = (int)(_pointNum * 2);

    // Copy points to GPU
    _pointVec.resize(_pointNum); // 1 additional slot for the infinity point
    _pointVec.copyFromHost(_input->InputPointVec);

    // Copy constraints to GPU
    _constraintVec.copyFromHost(_input->InputConstraintVec);
...
#pragma endregion
    ...
    kerMarkRejectedConsFlips<<<BlocksPerGrid, ThreadsPerBlock>>>(
        ...
        _input->isProfiling(ProfDiag) ? toKernelPtr(_rejFlipVec) : nullptr);
    CudaCheckError();
...
void GpuDel::splitTri()
{
    ...
    if (_availPtNum - _insNum < _insNum && _insNum < 0.1 * _pointNum)
    {
        _doFlipping = false;
...
#pragma endregion
    ...
    kerMarkRejectedFlips<<<BlocksPerGrid, ThreadsPerBlock>>>(toKernelPtr(_actTriVec),
                                                             ...
                                                             toKernelPtr(flipToTri),
                                                             orgActNum,
                                                             _input->isProfiling(ProfDiag) ? toKernelPtr(_rejFlipVec)
                                                                                           : nullptr);
    CudaCheckError();
...
void GpuDel::dispatchCheckDelaunay(CheckDelaunayMode checkMode, int orgActNum, IntDVec &triVoteVec)
{
    ...
    case CircleFastOrientFast:
        kerCheckDelaunayFast<<<BlocksPerGrid, ThreadsPerBlock>>>(
            ...
            toKernelPtr(triVoteVec),
            orgActNum,
            _input->isProfiling(ProfDiag) ? toKernelPtr(_circleCountVec) : nullptr);
        CudaCheckError();
        break;
...
void GpuDel::outputToHost()
{
    ...
    for (int ti = 0; ti < triNum; ++ti)
    {
        ...
        _output->segVec.insert(segArr, segArr + TriSegNum);
    }
