...
class HashTable
{
  public:
    ...
    explicit HashTable(const HashFunctor<KeyType> &hash) : _hash(hash), _size(0)
    {
        _table.resize(1);
    }
  ...
  private:
    ...
    void growTable()
    {
        if (_table.size() * HashFactor * Ratio > _size)
            return;
...
{
  public:
    int operator()(unsigned int key) const override;
};
...
class HashPoint2 : public HashFunctor<Point2>
{
  ...
  public:
    int operator()(Point2 p) const override;
};
