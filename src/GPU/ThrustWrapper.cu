...
class CachedAllocator
{
  ...
  public:
    ...
    CachedAllocator() : BlockSize(4096)
    {
    }
    ...
    void freeAll()
    {
        ...
        for (FreeBlocks::iterator i = freeBlocks.begin(); i != freeBlocks.end(); ++i)
        {
            cudaFree(i->second);
            totalSize += i->first;
        }

        for (AllocBlocks::iterator i = allocBlocks.begin(); i != allocBlocks.end(); ++i)
        {
            cudaFree(i->first);
            totalSize += i->second;
        }
...
void thrust_sort_by_key(
    ...
    thrust::zip_iterator<thrust::tuple<DevVector<int>::iterator, DevVector<Point2>::iterator>> valueBeg)
{
    thrust::sort_by_key(
...
void thrust_transform_GetMortonNumber(DevVector<Point2>::iterator inBeg,
                                      ...
                                      double                      maxVal)
{
    thrust::transform(thrust::cuda::par(thrustAllocator), inBeg, inEnd, outBeg, GetMortonNumber(minVal, maxVal));
}
...
int makeInPlaceMapAndSum(IntDVec &inVec)
{
    ...
    thrust::exclusive_scan(thrust::cuda::par(thrustAllocator), inVec.begin(), inVec.end(), inVec.begin());
...
int makeInPlaceIncMapAndSum(IntDVec &inVec)
{
    thrust::inclusive_scan(thrust::cuda::par(thrustAllocator), inVec.begin(), inVec.end(), inVec.begin());
...
int compactIfNegative(DevVector<int> &inVec, DevVector<int> &tempVec)
{
    tempVec.resize(inVec.size());

    tempVec.erase(thrust::copy_if(
                      thrust::cuda::par(thrustAllocator), inVec.begin(), inVec.end(), tempVec.begin(), IsNotNegative()),
                  tempVec.end());
...
void compactBothIfNegative(IntDVec &vec0, IntDVec &vec1)
{
    ...
    const IntZipDIter newEnd = thrust::remove_if(
        //thrust::cuda::par( thrustAllocator ),
...
void thrust_scan_TriHasVert(IntDVec &inVec, IntDVec &outVec)
{
    outVec.resize(inVec.size());

    thrust::transform_exclusive_scan(thrust::cuda::par(thrustAllocator),
                                     inVec.begin(),
                                     inVec.end(),
                                     outVec.begin(),
...
int thrust_copyIf_IsActiveTri(const CharDVec &inVec, IntDVec &outVec)
{
    ...
    outVec.erase(
        thrust::copy_if(thrust::cuda::par(thrustAllocator), first, last, inVec.begin(), outVec.begin(), IsTriActive()),
        outVec.end());

    return outVec.size();
}

int thrust_copyIf_IsNotNegative(const IntDVec &inVec, IntDVec &outVec)
{
    thrust::counting_iterator<int> first(0);
    thrust::counting_iterator<int> last = first + inVec.size();
    ...
    outVec.erase(thrust::copy_if(
                     thrust::cuda::par(thrustAllocator), first, last, inVec.begin(), outVec.begin(), IsNotNegative()),
                 outVec.end());

    return outVec.size();
}

int thrust_copyIf_TriHasVert(const IntDVec &inVec, IntDVec &outVec)
{
    thrust::counting_iterator<int> first(0);
    thrust::counting_iterator<int> last = first + inVec.size();

    outVec.erase(
        thrust::copy_if(thrust::cuda::par(thrustAllocator), first, last, inVec.begin(), outVec.begin(), IsTriHasVert()),
        outVec.end());

    return outVec.size();
}

void thrust_scatterSequenceMap(const IntDVec &inVec, IntDVec &outVec)
{
    thrust::counting_iterator<int> first(0);
    thrust::counting_iterator<int> last = first + inVec.size();
...
void thrust_scatterConstantMap(const IntDVec &inVec, CharDVec &outVec, char value)
{
...
void thrust_scan_TriAliveStencil(const CharDVec &inVec, IntDVec &outVec)
{
    outVec.resize(inVec.size());

    thrust::transform_inclusive_scan(thrust::cuda::par(thrustAllocator),
                                     inVec.begin(),
                                     inVec.end(),
                                     outVec.begin(),
                                     TriAliveStencil(),
...
int thrust_sum(const IntDVec &inVec)
{
