...
#include "../../include/GPU/KerPredWrapper.h"
...
__forceinline__ __device__ bool splitPoints(int        vertex,
                                            int       &vertTriIdx,
                                            const int *triToVert,
                                            Tri       *triArr,
                                            const int *insTriMap,
                                            ...
                                            int        insTriNum)
{
    ...
    if (triIdx == -1)
        return true; // Point inserted

    const int splitVertex = triToVert[triIdx];

    if (splitVertex >= INT_MAX - 1)
        return true; // Vertex's triangle will not be split in this round
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        Orient ort = (doFast) ? dPredWrapper.doOrient2DFast(triVert[fv[0]], triVert[fv[1]], vertex)
                              : dPredWrapper.doOrient2DFastExactSoS(triVert[fv[0]], triVert[fv[1]], vertex);

        if (doFast && (ort == OrientZero))
            return false; // Needs exact computation
...
__global__ void kerSplitPointsFast(KerIntArray vertTriVec,
                                   int        *triToVert,
                                   Tri        *triArr,
                                   int        *insTriMap,
                                   int        *exactCheckArr,
                                   ...
                                   int         insTriNum)
{
    ...
    {
        bool ret = splitPoints<true>(idx, vertTriVec._arr[idx], triToVert, triArr, insTriMap, triNum, insTriNum);
...
__global__ void kerSplitPointsExactSoS(int       *vertTriArr,
                                       int       *triToVert,
                                       Tri       *triArr,
                                       int       *insTriMap,
                                       const int *exactCheckArr,
                                       const int *counter,
                                       int        triNum,
                                       int        insTriNum)
{
    ...
    for (int idx = getCurThreadIdx(); idx < exactNum; idx += getThreadNum())
    {
        ...
        splitPoints<false>(vertIdx, vertTriArr[vertIdx], triToVert, triArr, insTriMap, triNum, insTriNum);
    }
}

template <CheckDelaunayMode checkMode>
__forceinline__ __device__ void checkDelaunayFast(const int *actTriArr,
                                                  Tri       *triArr,
                                                  TriOpp    *oppArr,
                                                  char      *triInfoArr,
                                                  int       *triVoteArr,
    ...
    for (int idx = getCurThreadIdx(); idx < actTriNum; idx += getThreadNum())
    {
        ...
        for (int botVi = 0; botVi < DEG; ++botVi)
                ...
                && !botOpp.isOppConstraint(botVi)) // or neighbor is a constraint
            {
                ...
                if (((botTi < topTi) || Checked == getTriCheckState(triInfoArr[topTi])))
        ...
        if (checkVi != 1)
        {
            ...
            for (; checkVi > 1; checkVi >>= 2)
            {
                ...
                if (SideIn != side)
                    continue; // No incircle failure at this face
            ...
            if (nullptr != dbgCircleCountArr)
                dbgCircleCountArr[botTi] = dbgCount;
        }
    }
}

__global__ void kerCheckDelaunayFast(int    *actTriArr,
                                     Tri    *triArr,
                                     TriOpp *oppArr,
                                     char   *triInfoArr,
                                     int    *triVoteArr,
                                     ...
                                     int    *dbgCircleCountArr)
{
    checkDelaunayFast<CircleFastOrientFast>(
        actTriArr, triArr, oppArr, triInfoArr, triVoteArr, nullptr, actTriNum, nullptr, dbgCircleCountArr);
}

__global__ void kerCheckDelaunayExact_Fast(int    *actTriArr,
                                           Tri    *triArr,
                                           TriOpp *oppArr,
                                           char   *triInfoArr,
                                           int    *triVoteArr,
                                           ...
                                           int    *dbgCircleCountArr)
{
    checkDelaunayFast<CircleExactOrientSoS>(
        actTriArr, triArr, oppArr, triInfoArr, triVoteArr, exactCheckVi, actTriNum, counter, dbgCircleCountArr);
}

__global__ void kerCheckDelaunayExact_Exact(Tri       *triArr,
                                            TriOpp    *oppArr,
                                            int       *triVoteArr,
                                            int2      *exactCheckVi,
                                            const int *counter,
...
template <bool doFast>
__forceinline__ __device__ bool relocatePoints(int vertex, int &location, const int *triToFlip, FlipItem *flipArr)
{
    ...
    while (flag == 1)
    {
        ...
        const Orient ord = doFast ? dPredWrapper.doOrient2DFast(flipItem._v[0], flipItem._v[1], vertex)
...
__global__ void
kerRelocatePointsFast(KerIntArray vertTriVec, int *triToFlip, FlipItem *flipArr, int *exactCheckArr, int *counter)
{
    // Iterate points
    for (int idx = getCurThreadIdx(); idx < vertTriVec._num; idx += getThreadNum())
    {
...
__global__ void
kerRelocatePointsExact(int *vertTriArr, int *triToFlip, FlipItem *flipArr, const int *exactCheckArr, const int *counter)
{
    const int exactNum = counter[CounterExact];
...
{
    if (tri.has(idx) || idx == dPredWrapper._infIdx)
    ...
    for (int i = 0; i < 3; ++i)
    {
        ...
        Orient ort = (doFast) ? dPredWrapper.doOrient2DFast(triVert[fv[0]], triVert[fv[1]], idx)
                              : dPredWrapper.doOrient2DFastExactSoS(triVert[fv[0]], triVert[fv[1]], idx);

        if (doFast && (ort == OrientZero))
            return -2; // Needs exact computation
...
__global__ void kerInitPointLocationFast(KerIntArray vertTriVec, int *exactCheckArr, int *counter, Tri tri)
{
    // Iterate points
    for (int idx = getCurThreadIdx(); idx < vertTriVec._num; idx += getThreadNum())
    {
...
__global__ void kerInitPointLocationExact(int *vertTriArr, const int *exactCheckArr, const int *counter, Tri tri)
{
    const int exactNum = counter[CounterExact];
...
__global__ void
kerVoteForPoint(KerIntArray vertexTriVec, Tri *triArr, int *vertCircleArr, int *triCircleArr, int noSample)
{
    const float rate = static_cast<float>(vertexTriVec._num) / static_cast<float>(noSample);

    // Iterate uninserted points
    for (int idx = getCurThreadIdx(); idx < noSample; idx += getThreadNum())
    {
        const auto vert = static_cast<int>((static_cast<float>(idx) * rate));
        ...
        const Tri tri  = triArr[triIdx];
        auto      cval = static_cast<float>(dPredWrapper.inCircleDet(tri, vert));
...
template <bool doFast>
__forceinline__ __device__ void markTriConsIntersection(KerIntArray actConsVec,
                                                        Edge       *constraintArr,
                                                        Tri        *triArr,
                                                        TriOpp     *oppArr,
                                                        char       *triInfoArr,
                                                        const int  *vertTriArr,
                                                        int        *triConsArr,
                                                        int        *counter)
{
    ...
    for (int idx = getCurThreadIdx(); idx < actConsVec._num; idx += getThreadNum())
    {
        ...
        if (!doFast)
        {
            ...
            {
                actConsVec._arr[idx] = consIdx = makePositive(consIdx);
            }
        }

        Edge constraint = constraintArr[consIdx];
        ...
        while (true)
        {
            ...
            {
                or2 = doFast ? dPredWrapper.doOrient2DFast(constraint._v[0], constraint._v[1], nextVert)
                             : dPredWrapper.doOrient2DFastExactSoS(constraint._v[0], constraint._v[1], nextVert);

                if (doFast && or2 == OrientZero)
                    break;
        ...
        while (side != 3)
        {
            ...
            {
                Orient ori = doFast ? dPredWrapper.doOrient2DFast(constraint._v[0], constraint._v[1], nextVert)
                                    : dPredWrapper.doOrient2DFastExactSoS(constraint._v[0], constraint._v[1], nextVert);

                if (doFast && ori == OrientZero)
                    break;
...
__global__ void kerMarkTriConsIntersectionFast(KerIntArray actConsVec,
                                               Edge       *constraintArr,
                                               Tri        *triArr,
                                               TriOpp     *oppArr,
                                               char       *triInfoArr,
                                               ...
                                               int        *counter)
{
    markTriConsIntersection<true>(
        actConsVec, constraintArr, triArr, oppArr, triInfoArr, vertTriArr, triConsArr, counter);
}

__global__ void kerMarkTriConsIntersectionExact(KerIntArray actConsVec,
                                                Edge       *constraintArr,
                                                Tri        *triArr,
                                                TriOpp     *oppArr,
                                                char       *triInfoArr,
...
updatePairStatus(int triIdx, const int *triConsVec, Tri *triArr, TriOpp *oppArr, char *triInfoArr)
{
    if (doFast)
    {
        if (getTriCheckState(triInfoArr[triIdx]) != Changed)
    ...
    if (doFast && label < 0)
        return true; // No longer intersect constraints, at least for this round
    ...
    if (doFast && consIdx != oppConsIdx)
        return true; // Different labels
    ...
    Orient or1 = doFast ? dPredWrapper.doOrient2DFast(tri._v[vi], oppVert, tri._v[(vi + 1) % 3])
                        : dPredWrapper.doOrient2DFastExactSoS(tri._v[vi], oppVert, tri._v[(vi + 1) % 3]);
    Orient or2 = doFast ? dPredWrapper.doOrient2DFast(tri._v[vi], oppVert, tri._v[(vi + 2) % 3])
                        : dPredWrapper.doOrient2DFastExactSoS(tri._v[vi], oppVert, tri._v[(vi + 2) % 3]);

    // Need exact computation?
    if (doFast && (or1 == OrientZero || or2 == OrientZero))
        return false;
...
__global__ void kerUpdatePairStatusFast(KerIntArray actTriVec,
                                        int        *triConsVec,
                                        Tri        *triArr,
                                        TriOpp     *oppArr,
                                        char       *triInfoArr,
...
__global__ void kerUpdatePairStatusExact(KerIntArray actTriVec,
                                         int        *triConsVec,
                                         Tri        *triArr,
                                         TriOpp     *oppArr,
                                         char       *triInfoArr,
...
template <bool doFast>
__forceinline__ __device__ bool
checkConsFlipping(int midIdx, const int *triConsArr, char *triInfoArr, Tri *triArr, TriOpp *oppArr, int *triVoteArr)
{
    PairType midType;

    if (doFast)
    {
        const auto triInfo = triInfoArr[midIdx];
        midType            = getTriPairType(triInfo);

        if (getTriCheckState(triInfo) != Changed)
            return true; // Nothing changed.

        setTriCheckState(triInfoArr[midIdx], Checked);

        if (midType == PairNone || midType == PairConcave)
    ...
    if (doFast && (leftLabel < 0 || decode_cIdx(leftLabel) != decode_cIdx(midLabel)))
        return true;
    ...
    Orient or1 = doFast ? dPredWrapper.doOrient2DFast(tri._v[leftVi], rightVert, tri._v[(leftVi + 1) % 3])
                        : dPredWrapper.doOrient2DFastExactSoS(tri._v[leftVi], rightVert, tri._v[(leftVi + 1) % 3]);
    Orient or2 = doFast ? dPredWrapper.doOrient2DFast(tri._v[leftVi], rightVert, tri._v[(leftVi + 2) % 3])
                        : dPredWrapper.doOrient2DFastExactSoS(tri._v[leftVi], rightVert, tri._v[(leftVi + 2) % 3]);

    if (doFast && (or1 == OrientZero || or2 == OrientZero))
        return false; // Need exact check

    if (or1 == or2)
        return true; // Concave, neither Case 2 nor 3
...
__global__ void kerCheckConsFlippingFast(KerIntArray actTriVec,
                                         int        *triConsArr,
                                         char       *triInfoArr,
                                         Tri        *triArr,
                                         TriOpp     *oppArr,
...
__global__ void kerCheckConsFlippingExact(int       *triConsArr,
                                          char      *triInfoArr,
                                          Tri       *triArr,
                                          TriOpp    *oppArr,
                                          int       *triVoteArr,
